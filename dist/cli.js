...
async function main() {
	...
	let fixtures;
	try {
		isDir = statSync(fixturePath).isDirectory();
		if (isDir) fixtures = loadFixturesFromDir(fixturePath, logger);
		else fixtures = loadFixtureFile(fixturePath, logger);
	} catch (err) {
	...
	let watcher = null;
	if (watchMode) {
		watcher = watchFixtures(fixturePath, fixtures, isDir ? () => loadFixturesFromDir(fixturePath, logger) : () => loadFixtureFile(fixturePath, logger), {
			logger,
