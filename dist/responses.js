import { flattenHeaders, generateId, generateToolCallId, isErrorResponse, isTextResponse, isToolCallResponse } from "./helpers.js";
...
async function handleResponses(req, res, raw, fixtures, journal, defaults, setCorsHeaders) {
	...
	if (!fixture) {
		if (defaults.record) {
			if (await proxyAndRecord(req, res, completionReq, "openai", req.url ?? "/v1/responses", fixtures, defaults, raw)) {
				journal.add({
					method: req.method ?? "POST",
					path: req.url ?? "/v1/responses",
					headers: flattenHeaders(req.headers),
...
//#endregion
export { buildTextStreamEvents, buildToolCallStreamEvents, handleResponses, responsesToCompletionRequest };
